 ...
 * to Bifrost Kusama (Parachain) account, where the `xcmVersion` is set to 3, and `isLimited` is false declaring that
...
const main = async () => {
	...
	try {
		callInfo = await assetApi.createTransferTransaction(
			...
			['KMA'], // Note: since it is the primary asset of Moonriver that is being sent to a Parachain, it will be a `reserveTransfer` call
			['1000000000000'],
			{
				format: 'call',
				isLimited: false,
