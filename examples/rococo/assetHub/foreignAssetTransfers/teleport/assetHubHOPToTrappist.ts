...
/**
 * In this example we are creating a `polkadotXcm` pallet `transferAssets` call to send HOP (foreign asset with location `{"parents":"1","interior":{"X1":{"Parachain":"1836"}}}`)
 * from a Rococo Asset Hub (System Parachain) account
 * to a Trappist Testnet (ParaChain) account, where the `xcmVersion` is set to 4, and there is no
