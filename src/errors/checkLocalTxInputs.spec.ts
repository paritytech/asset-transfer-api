...
describe('checkLocalTxInput', () => {
	...
	it('Should correctly return Balances with a native token', async () => {
		const res = await checkLocalTxInput(systemAssetsApi._api, ['KSM'], ['10000'], specName, registry, false, false);
		expect(res).toEqual('Balances');
	});
	...
	it('Should correctly return Assets with a valid assetId', async () => {
		const res = await checkLocalTxInput(systemAssetsApi._api, ['1984'], ['10000'], specName, registry, false, false);
		expect(res).toEqual('Assets');
	});
	it('Should correctly throw an error for incorrect length on `assetIds`', async () => {
		await expect(async () => {
			await checkLocalTxInput(systemAssetsApi._api, ['1', '2'], ['10000'], specName, registry, false, false);
	...
	it('Should correctly throw an error for incorrect length on `amounts`', async () => {
		await expect(async () => {
			await checkLocalTxInput(systemAssetsApi._api, ['1'], ['10000', '20000'], specName, registry, false, false);
		}).rejects.toThrowError(
			'Local transactions must have the `assetIds` input be a length of 1 or 0, and the `amounts` input be a length of 1'
		);
	});
	it('Should correctly throw an error with an incorrect assetId', async () => {
		await expect(async () => {
			await checkLocalTxInput(systemAssetsApi._api, ['TST'], ['10000'], specName, registry, false, false);
	...
	it("Should correctly throw an error when the integer assetId doesn't exist", async () => {
		await expect(async () => {
			await checkLocalTxInput(systemAssetsApi._api, ['9876111'], ['10000'], specName, registry, false, false);
	...
	it('Should correctly throw an error when the given multilocation assetIds is empty', async () => {
		...
		await expect(async () => {
			await checkLocalTxInput(systemAssetsApi._api, [], ['10000'], specName, registry, true, false);
		}).rejects.toThrowError(expectedError);
	});
});
